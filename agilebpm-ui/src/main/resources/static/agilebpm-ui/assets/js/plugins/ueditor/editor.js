        ...
        initEditorUI:function () {
            ...
            var popup = new baidu.editor.ui.Popup({
                ...
                _onCustomerDialogClick:function(){
                	this.hide();
                	var child=popup.anchorEl.children[0];
                	if(child){
            			editor.curInput=child;
            ...
            if (editor.options.imagePopup) {
                editor.addListener('mouseover', function (t, evt) {
    	    		...
    	    		var ifEl=el.hasAttribute("lablename");
    	    		var ifElParent = el.parentElement.getAttribute("name") =="editable-input";
    	    		
    	    		var isCustomerDialog = el.hasAttribute("ht-customerDialog");
    	    		...
    	    		if(ifEl||ifElParent ||isCustomerDialog ||isNgInput){
    	    			if(readOnly){
    	    				if(isflag){
    				   ...
    				   if(ifInput){   				   	 
		    				  html = popup.formatHtml(
		    	                    ...
		    	                    '<span onclick="$$._onInputEditButtonClick(event, this);" class="edui-clickable">' + editor.getLang("modify") + '</span>&nbsp;&nbsp;'+
		    	                    '<span onclick="$$._onInputCutButtonClick(event, this);" class="edui-clickable">剪切</span>&nbsp;&nbsp;'+
		    	                    '<span onclick="$$._onInputDialogButtonClick(event, this);" class="edui-clickable">查询设置</span>&nbsp;&nbsp;' +
		    	                    '<span onclick="$$._onInputImportExcelButtonClick(event, this);" class="edui-clickable">导入Excel</span>&nbsp;&nbsp;' +
		    	                    '<span onclick="$$._onInputExportExcelButtonClick(event, this);" class="edui-clickable">导出Excel</span>&nbsp;&nbsp;' +
		    	                       '</nobr>' );
    				   }
    				   if(onlydelete){
    					   html = popup.formatHtml(
    				   ...
    				   if(isCustomerDialog){
    					   html = popup.formatHtml(
	    	                       '<nobr><span onclick="$$._onCustomerDialogClick(event, this);" class="edui-clickable">设置自定义对话框</span>&nbsp;&nbsp;' +
	    	                       '<span onclick="$$._onUnCustomerDialogClick(event, this);" class="edui-clickable">移除自定义对话框</span>&nbsp;&nbsp;'+
	    	                       '</nobr>' );
    				   }
    				   if(isNgInput){
    					   html = popup.formatHtml(
	    	                       '<nobr>'+
	    	                       '<span onclick="$$._onRemoveInputClick(event, this);" class="edui-clickable">移除</span>&nbsp'+
	    	                       '</nobr>' );
    				   }
                ...
                editor.addListener('selectionchange', function (t, causeByUi) {
                    ...
                    if (editor.ui._dialogs.linkDialog) {
                        ...
                        if(isWordTemplate){
    					   html = popup.formatHtml(
    		                       '<nobr>'+editor.getLang("property")+': <span onclick="$$._onWordTemplateButtonClick();" class="edui-clickable">修改</span>&nbsp;&nbsp;</nobr>' );
